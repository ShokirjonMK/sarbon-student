...
import { Alert, Button, Empty, Modal, Spin, Tabs, message } from 'antd';
...
const SubjectExam: React.FC<{semestrSubject:any, semestrSubjectFetching: boolean}> = ({semestrSubject, semestrSubjectFetching}): JSX.Element => {

  const navigate = useNavigate()
  const [selectedTestStart, setselectedTestStart] = useState<any>();
  const [isShowValidIPAlert, setisShowValidIPAlert] = useState<boolean>(false);
  ...
  const { mutate: startTest, isLoading: startLoading } = useMutation({
    ...
    onSuccess: async (res) => {
      if (res?.status === 1) {
        ...
        setisShowValidIPAlert(false)
        navigate(`/exam-student/${res?.data?.id}`)
      } else {
  ...
  const items2: TabsProps['items'] = semestrSubject?.data?.finalExamStudent?.filter((i: any) => i?.exams_type_id === 3)?.map((item: any, index: number) => ({
    key: `${index + 1}`,
    label: [
      {id: 1, name: "1 shakl"},
      {id: 2, name: "1 A shakl"},
    ...
    children: item?.finalExamTestStudent?.status === 1 ? <div className='grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5'>
      {
        item?.finalExamTestStudent?.finalExamTestStart?.map((testStart: any, i: number) => (  
          <ExamCard index={i} testStart={testStart} item={item} setselectedTestStart={setselectedTestStart} url={'exam-student'} />
        ))
      }
    </div> : <div></div>
  }));
  ...
  return (
    <Spin spinning={semestrSubjectFetching}>
      ...
      <Modal 
        ...
        footer={false}
        onCancel={() => {setselectedTestStart(undefined); setisShowValidIPAlert(false)}}
      >
        ...
        {
          isShowValidIPAlert ? <Alert description={"Sizning qurilmangizga ruxsat berilmagan. Iltimos imtihon uchun belgilangan universitet kompyuteridan foydalaning!"} className='my-4 text-[17px]' type='error' /> : ""
        }
